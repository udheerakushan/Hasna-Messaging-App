  ...
  Future<void> _showImageSourceChoice() async {
    final choice = await showModalBottomSheet<String>(context: context, builder: (ctx) {
      return SafeArea(
        child: Wrap(
          children: [
            ListTile(leading: const Icon(Icons.photo_camera), title: const Text('Camera'), onTap: () => Navigator.of(ctx).pop('camera')),
            ListTile(leading: const Icon(Icons.photo_library), title: const Text('Gallery'), onTap: () => Navigator.of(ctx).pop('gallery')),
            ListTile(leading: const Icon(Icons.close), title: const Text('Cancel'), onTap: () => Navigator.of(ctx).pop(),),
          ],
        ),
      );
    });
    ...
    if (m['type'] == 'image' || m['type'] == 'image_encrypted') {
      ...
      try {
        ...
        if (m['type'] == 'image_encrypted') {
          final me = KeyStorage.getProfile();
          // Since getProfile returns Future, but here we're in sync flow, we handle lazily by attempting to decode later. For now, try to show raw bytes as fallback.
          clear = bytes;
        } else {
          clear = bytes;
        }
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                IconButton(icon: const Icon(Icons.image), onPressed: _showImageSourceChoice),
                Expanded(
                  child: TextField(
